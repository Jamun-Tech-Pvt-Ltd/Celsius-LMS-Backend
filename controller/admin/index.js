...
const adminResolversQuery = {
  ...
  getAdminDashboard: async (_, args, { userId, role, platform }) => {
    ...
    if (platform === 'internal') {
      ...
      const total_active_student = await prisma.jmkstdinfo.count({ where: { company_id: admin.company_id, lastSeen: { gte: fiveMinutesAgo } } });
      const total_active_trainer = await prisma.jmktrinfo.count({ where: { company_id: admin.company_id, lastSeen: { gte: fiveMinutesAgo } } });
      const total_active_admin = await prisma.jmkuserinfo.count({ where: { company_id: admin.company_id, lastSeen: { gte: fiveMinutesAgo } } });
      data.total_admins = total_admins;
      data.total_student = student;
      data.total_trainer = trainer;
      data.total_user = student + trainer + total_admins;
